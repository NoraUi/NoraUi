...
public class MaterialSteps extends Step {
     ...
     *             if the scenario encounters a functional error
     */
    protected void selectMatSlideToggle(PageElement element, boolean checked, Object... args)
            throws TechnicalException, FailureException {
        try {
            final WebElement webElement = Wait
                    .until(ExpectedConditions.elementToBeClickable(Utilities.getLocator(element, args)));
            if ("false".equals(webElement.getAttribute("ng-reflect-checked")) && checked
                    || "true".equals(webElement.getAttribute("ng-reflect-checked")) && !checked) {
                webElement.click();
            }
        } catch (final Exception e) {
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_MATERIAL_UNABLE_TO_CHECK_ELEMENT),
                            element, element.getPage().getApplication()),
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateMatRadioButton(PageElement pageElement, String valueOrKey)
            throws TechnicalException, FailureException {
        ...
        try {
            ...
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_MATERIAL_UNABLE_TO_SELECT_RADIO_BUTTON),
                            pageElement),
