...
public abstract class Step implements IStep {
    ...
    protected void clickOn(PageElement toClick, Object... args) throws TechnicalException, FailureException {
        displayMessageAtTheBeginningOfMethod("clickOn: %s in %s", toClick.toString(),
                toClick.getPage().getApplication());
        try {
            ...
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_OPEN_ON_CLICK), toClick,
                            toClick.getPage().getApplication()),
                    true, toClick.getPage().getCallBack());
        }
    }
    ...
    protected void clickOnByJs(PageElement toClick, Object... args) throws TechnicalException, FailureException {
        displayMessageAtTheBeginningOfMethod("clickOnByJs: %s in %s", toClick.toString(),
                toClick.getPage().getApplication());
        try {
            Wait.until(ExpectedConditions.elementToBeClickable(Utilities.getLocator(toClick, args)));
            ((JavascriptExecutor) getDriver()).executeScript("document.evaluate(\""
                    + StringEscapeUtils.escapeJava(Utilities.getLocatorValue(toClick, args))
    ...
    protected void clickOnByJs(Page page, String xpath) throws TechnicalException, FailureException {
        ...
        try {
            ...
            ((JavascriptExecutor) getDriver()).executeScript("document.evaluate(\"" + xpath
                    + "\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click();");
        } catch (final Exception e) {
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_EVALUATE_XPATH), xpath,
                            page.getApplication()),
                    true, page.getCallBack());
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateText(PageElement pageElement, String textOrKey, Object... args)
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateText(PageElement pageElement, String textOrKey, CharSequence keysToSend, Object... args)
            throws TechnicalException, FailureException {
        String value = getTextOrKey(textOrKey);
        if (!"".equals(value)) {
            try {
                Wait.untilAnd(ExpectedConditions.elementToBeClickable(Utilities.getLocator(pageElement, args)))
                        .map((WebElement elem) -> {
                            elem.clear();
                            return elem;
                        }).then((elem) -> {
            ...
            log.debug(
                    "Empty data provided. No need to update text. If you want clear data, you need use: \"I clear text in ...\"");
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void clearText(PageElement pageElement, CharSequence keysToSend, Object... args)
            throws TechnicalException, FailureException {
        try {
            final WebElement element = Wait
                    .until(ExpectedConditions.presenceOfElementLocated(Utilities.getLocator(pageElement, args)));
            element.clear();
            if (keysToSend != null) {
            ...
            new Result.Failure<>(
                    e.getMessage(), Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_ERROR_CLEAR_ON_INPUT),
                            pageElement, pageElement.getPage().getApplication()),
                    true, pageElement.getPage().getCallBack());
        }
    }
     ...
     */
    protected boolean checkInputText(PageElement pageElement, String textOrKey)
        ...
        return !(inputText == null || value == null || inputText.getAttribute(VALUE) == null
                || !value.equals(inputText.getAttribute(VALUE).trim()));
    }
    ...
    protected void expectText(PageElement pageElement, String textOrKey) throws FailureException, TechnicalException {
        ...
        try {
            ...
            new Result.Failure<>(element == null ? null : element.getText(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_WRONG_EXPECTED_VALUE), pageElement,
                            textOrKey.startsWith(cryptoService.getPrefix()) ? SECURE_MASK : value,
                            pageElement.getPage().getApplication()),
                    true, pageElement.getPage().getCallBack());
        }
    ...
    protected boolean checkMandatoryTextField(PageElement pageElement) throws FailureException {
        ...
        try {
            ...
            new Result.Failure<>(pageElement, Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_FIND_ELEMENT), true,
    ...
    protected String readValueTextField(PageElement pageElement) throws FailureException {
        try {
            return Wait.until(ExpectedConditions.presenceOfElementLocated(Utilities.getLocator(pageElement)))
                    .getAttribute(VALUE);
        } catch (final Exception e) {
            new Result.Failure<>(e.getMessage(), Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_FIND_ELEMENT),
                    true, pageElement.getPage().getCallBack());
     ...
     *             if the scenario encounters a functional error
     */
    protected void checkText(PageElement pageElement, String textOrKey, Object... args)
            throws TechnicalException, FailureException {
        ...
        if (log.isDebugEnabled()) {
            log.debug("checkText() expected [{}] and found [{}].",
                    textOrKey.startsWith(cryptoService.getPrefix()) ? SECURE_MASK : value, innerText);
        }
        if (!value.equals(innerText)) {
            new Result.Failure<>(innerText,
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_WRONG_EXPECTED_VALUE), pageElement,
                            textOrKey.startsWith(cryptoService.getPrefix()) ? SECURE_MASK : value,
                            pageElement.getPage().getApplication()),
                    true, pageElement.getPage().getCallBack());
     ...
     *             if the scenario encounters a functional error
     */
    protected void checkTextContains(PageElement pageElement, String textOrKey, Object... args)
            throws TechnicalException, FailureException {
        WebElement webElement = null;
        String value = getTextOrKey(textOrKey);
        try {
        ...
        final String innerText = webElement == null ? null : webElement.getText();
        if (log.isDebugEnabled()) {
            log.debug("checkTextContains() expected [{}] and found [{}].",
                    textOrKey.startsWith(cryptoService.getPrefix()) ? SECURE_MASK : value, innerText);
        }
     ...
     *             (with screenshot, with exception)
     */
    protected void checkElementVisible(PageElement pageElement, boolean displayed, Object... args)
            throws FailureException {
        if (displayed) {
            try {
                Wait.until(ExpectedConditions.visibilityOfElementLocated(Utilities.getLocator(pageElement, args)));
            } catch (final Exception e) {
                new Result.Failure<>(e.getMessage(), Messages.getMessage(Messages.FAIL_MESSAGE_ELEMENT_STILL_VISIBLE),
                        true, pageElement.getPage().getCallBack());
            }
        } else {
            try {
                Wait.until(ExpectedConditions
                        .not(ExpectedConditions.visibilityOfElementLocated(Utilities.getLocator(pageElement))));
            } catch (final Exception e) {
                new Result.Failure<>(e.getMessage(), Messages.getMessage(Messages.FAIL_MESSAGE_ELEMENT_STILL_VISIBLE),
                        true, pageElement.getPage().getCallBack());
            }
     ...
     *             (with screenshot, with exception)
     */
    protected void checkElementPresence(PageElement pageElement, boolean present, Object... args)
            throws FailureException {
        if (present) {
            try {
            ...
            try {
                Wait.until(ExpectedConditions.not(
                        ExpectedConditions.presenceOfAllElementsLocatedBy(Utilities.getLocator(pageElement, args))));
            } catch (final Exception e) {
                new Result.Failure<>(e.getMessage(), Messages.getMessage(Messages.FAIL_MESSAGE_ELEMENT_STILL_VISIBLE),
                        true, pageElement.getPage().getCallBack());
            }
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateList(PageElement pageElement, String textOrKey, Object... args)
            throws TechnicalException, FailureException {
        String value = getTextOrKey(textOrKey);
        try {
     ...
     *             if the scenario encounters a functional error
     */
    protected boolean checkTextSelectedInList(PageElement pageElement, String text, Object... args)
            throws FailureException {
        try {
            final WebElement select = Wait
                    .until(ExpectedConditions.elementToBeClickable(Utilities.getLocator(pageElement, args)));
            final Select dropDown = new Select(select);
            return text.equalsIgnoreCase(dropDown.getFirstSelectedOption().getText());
        } catch (final Exception e) {
            new Result.Failure<>(text, Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_FIND_ELEMENT), true,
                    pageElement.getPage().getCallBack());
            return false;
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateDateValidated(PageElement pageElement, String dateType, String date, Object... args)
            throws TechnicalException, FailureException {
        ...
        try {
            ...
            } else if (formatter.format(today).equals(date)
                    && ("future".equals(dateType) || "today".equals(dateType))) {
                log.debug("update Date with today");
                updateText(pageElement, date, args);
            } else if (valideDate.after(Calendar.getInstance().getTime())
                    && ("future".equals(dateType) || "future_strict".equals(dateType))) {
                log.debug("update Date with a date after today: {}", date);
                updateText(pageElement, date, args);
            } else {
                new Result.Failure<>(date,
                        Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNEXPECTED_DATE),
                                Messages.getMessage(Messages.DATE_GREATER_THAN_TODAY)),
                        true, pageElement.getPage().getCallBack());
            }
        } catch (final ParseException e) {
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_WRONG_DATE_FORMAT), pageElement, date),
                    false, pageElement.getPage().getCallBack());
        }
     ...
     *             if the scenario encounters a functional error
     */
    protected void saveElementValue(PageElement pageElement, Object... args)
            throws TechnicalException, FailureException {
     ...
     *             if the scenario encounters a functional error
     */
    protected void saveElementValue(PageElement pageElement, String targetKey, Object... args)
            throws TechnicalException, FailureException {
        log.debug("saveElementValue: {} to {} in {}.", pageElement, targetKey, pageElement.getPage().getApplication());
        String txt = "";
        try {
            final WebElement elem = Wait
                    .until(ExpectedConditions.presenceOfElementLocated(Utilities.getLocator(pageElement, args)));
        ...
        try {
            ...
            new Result.Failure<>(
                    e.getMessage(), Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_RETRIEVE_VALUE),
                            pageElement, pageElement.getPage().getApplication()),
                    true, pageElement.getPage().getCallBack());
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateRadioList(PageElement pageElement, String valueKeyOrKey, Map<String, String> printedValues)
            throws TechnicalException, FailureException {
        final String valueKey = Context.getValue(valueKeyOrKey) != null ? Context.getValue(valueKeyOrKey)
                : valueKeyOrKey;
        try {
            final List<WebElement> radioButtons = Wait
                    .until(ExpectedConditions.presenceOfAllElementsLocatedBy(Utilities.getLocator(pageElement)));
            ...
            new Result.Failure<>(e.getMessage(), Messages
                    .format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_SELECT_RADIO_BUTTON), pageElement),
                    true, pageElement.getPage().getCallBack());
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void updateRadioList(PageElement pageElement, String valueOrKey)
            throws TechnicalException, FailureException {
    ...
    protected void passOver(PageElement element) throws TechnicalException, FailureException {
        try {
            ...
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_PASS_OVER_ELEMENT), element,
                            element.getPage().getApplication()),
                    true, element.getPage().getCallBack());
        }
     ...
     *             if the scenario encounters a functional error
     */
    protected void selectCheckbox(PageElement element, boolean checked, Object... args)
            throws TechnicalException, FailureException {
        try {
            final WebElement webElement = Wait
                    .until(ExpectedConditions.elementToBeClickable(Utilities.getLocator(element, args)));
            if (webElement.isSelected() != checked) {
                webElement.click();
            }
        } catch (final Exception e) {
            new Result.Failure<>(e.getMessage(),
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_CHECK_ELEMENT), element,
                            element.getPage().getApplication()),
                    true, element.getPage().getCallBack());
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void selectCheckbox(PageElement element, String valueKeyOrKey, Map<String, Boolean> values)
            throws TechnicalException, FailureException {
        final String valueKey = Context.getValue(valueKeyOrKey) != null ? Context.getValue(valueKeyOrKey)
                : valueKeyOrKey;
        try {
            final WebElement webElement = Wait
                    .until(ExpectedConditions.elementToBeClickable(Utilities.getLocator(element)));
    ...
    protected void switchFrame(PageElement element, Object... args) throws FailureException, TechnicalException {
        try {
            ...
            new Result.Failure<>(element,
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UNABLE_TO_SWITCH_FRAME), element,
                            element.getPage().getApplication()),
                    true, element.getPage().getCallBack());
        }
    }
     ...
     *             if the scenario encounters a functional error
     */
    protected void uploadFile(PageElement pageElement, String fileOrKey, Object... args)
            throws TechnicalException, FailureException {
        final String path = Context.getValue(fileOrKey) != null ? Context.getValue(fileOrKey)
                : System.getProperty(USER_DIR) + File.separator + DOWNLOADED_FILES_FOLDER + File.separator + fileOrKey;
        if (!"".equals(path)) {
            try {
                final WebElement element = Wait
                        .until(ExpectedConditions.presenceOfElementLocated(Utilities.getLocator(pageElement, args)));
                ...
                new Result.Failure<>(e.getMessage(),
                        Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_UPLOADING_FILE), path), true,
                        pageElement.getPage().getCallBack());
            }
        } else {
            log.debug(
     ...
     */
    protected void displayMessageAtTheBeginningOfMethod(String methodName, String act, String concernedActivity,
            List<String> concernedActivities) {
        log.debug("{} {}: {} with {} concernedActivity(ies)", act, methodName, concernedActivity,
                concernedActivities.size());
        int i = 0;
     ...
     *            is a list of authorized activities
     */
    protected void displayMessageAtTheBeginningOfMethod(String methodName, String act,
            List<String> concernedActivities) {
    ...
    protected void checkAlert(Page page) throws FailureException {
        if (!DriverFactory.IE.equals(Context.getBrowser())) {
            ...
            if (txt != null) {
                new Result.Failure<>(txt, Messages.getMessage(Messages.FAIL_MESSAGE_ALERT_FOUND), true,
                        page.getCallBack());
            }
        } else {
    ...
    private Function<GherkinConditionedLoopedStep, GherkinConditionedLoopedStep> buildConditionsList(
            List<GherkinStepCondition> stepConditions) {
        return f -> {
    ...
    private Function<SimpleEntry<Method, List<?>>, SimpleEntry<Method, List<?>>> invokeMethodWithConditions(
            List<GherkinStepCondition> stepConditions) {
        return f -> {
            Object[] tab;
            ...
            try {
                f.getKey().invoke(NoraUiInjector.getNoraUiInjectorSource().getInstance(f.getKey().getDeclaringClass()),
                        tab);
            } catch (final Exception e) {
    ...
    private void displayMessageAtTheBeginningOfMethod(String message, String element, String application)
            throws TechnicalException {
        try {
    ...
    private void setDropDownValue(PageElement element, String text, Object... args)
            throws TechnicalException, FailureException {
        ...
        if (index != -1) {
            ...
            new Result.Failure<>(text.startsWith(cryptoService.getPrefix()) ? SECURE_MASK : text,
                    Messages.format(Messages.getMessage(Messages.FAIL_MESSAGE_VALUE_NOT_AVAILABLE_IN_THE_LIST), element,
                            element.getPage().getApplication()),
                    false, element.getPage().getCallBack());
        }
    }
