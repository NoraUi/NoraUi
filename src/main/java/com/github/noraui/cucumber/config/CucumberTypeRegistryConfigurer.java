...
public class CucumberTypeRegistryConfigurer implements TypeRegistryConfigurer {
    ...
    public static List<DataTableType> getDataTableTypes() {
        if (dataTableTypes.isEmpty()) {
            dataTableTypes = Arrays.asList(new DataTableType(Boolean.class, Boolean::parseBoolean),
                    new DataTableType(GherkinStepCondition.class, new TableEntryTransformer<GherkinStepCondition>() {
                        @Override
                        public GherkinStepCondition transform(Map<String, String> entry) {
                            ...
                            return new GherkinStepCondition(entry.get("key"),
                                    entry.get("expected") != null ? entry.get("expected") : "",
                                    entry.get("actual") != null ? entry.get("actual") : "");
                        }
                    }), new DataTableType(GherkinConditionedLoopedStep.class,
                            new TableEntryTransformer<GherkinConditionedLoopedStep>() {
                                @Override
                                public GherkinConditionedLoopedStep transform(Map<String, String> entry) {
                                    log.debug("configureTypeRegistry GherkinConditionedLoopedStep key:{}",
                                            entry.get("key"));
                                    log.debug("configureTypeRegistry GherkinConditionedLoopedStep step:{}",
                                            entry.get("step"));
                                    log.debug("configureTypeRegistry GherkinConditionedLoopedStep expected:{}",
                                            entry.get("expected"));
                                    log.debug("configureTypeRegistry GherkinConditionedLoopedStep actual:{}",
                                            entry.get("actual"));
                                    return new GherkinConditionedLoopedStep(entry.get("key"), entry.get("step"),
                                            entry.get("expected") != null ? entry.get("expected") : "",
                                            entry.get("actual") != null ? entry.get("actual") : "");
                                }
                            }));
    ...
    public static List<ParameterType<?>> getParameterTypes() {
        if (parameterTypes.isEmpty()) {
            parameterTypes = Arrays.asList(new ParameterType<PageElement>("page-element", "\\'(.*\\..*-.*)\\'",
                    PageElement.class, (final String strPageElement) -> {
                        ...
                        try {
                            ...
                            throw new TechnicalException(Messages.format(
                                    Messages.getMessage(Page.UNABLE_TO_RETRIEVE_PAGE_ELEMENT), page, elementName), e);
                        }
                        ...
                        try {
                            ...
                            throw new TechnicalException(
                                    Messages.format(Messages.getMessage(Page.UNABLE_TO_RETRIEVE_PAGE), strPage), e);
                        }
                    }), new ParameterType<Boolean>("is-isnot", "(is( not)?)|(est|n\\'est pas)", Boolean.class,
                            (final String not) -> {
                                return Boolean.valueOf(not.split(" ").length > 1);
                            }),
                    new ParameterType<Boolean>("should-shouldnot",
                            "(should( not)?)|(devrait|devraient|ne devrait pas|ne devraient pas)", Boolean.class,
                            (final String not) -> {
                                return Boolean.valueOf(not.split(" ").length > 1);
                            }));
        }
