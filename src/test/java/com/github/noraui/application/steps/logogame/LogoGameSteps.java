...
public class LogoGameSteps extends Step {
	...
	public void checkDemoPortalPage() throws FailureException {
		if (!this.logoGamePage.checkPage()) {
			new Result.Failure<>("LOGOGAME", Messages.getMessage(Messages.FAIL_MESSAGE_UNKNOWN_CREDENTIALS), true,
	...
	/**
	 * A check that all brands are not prohibited, because any minors can not play
	 * with alcohol and tobacco brands.
	 *
	 * @param jsonLogos Serialized Json representation of all logos (all brands)
	 * @throws TechnicalException is throws if you have a technical error (format,
	 *                            configuration, datas, ...) in NoraUi.
		...
		for (int i = 0; i < logos.size(); i++) {
			if (ProhibitedBrands.getAlcool().contains(logos.get(i).getBrand())
				...
				new Result.Failure<>(logos.get(i).getBrand(),
						...
						logos.get(i).getNid(), Context.getCallBack(Callbacks.RESTART_WEB_DRIVER));
			}
		}
	}
